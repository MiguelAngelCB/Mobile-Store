...
public class ValidatorLengthComposite {
  ...
  public HashMap<String, ErrorValidate> validate(Recover recover) {
    addValidate(Validator.size.getName(), recover.getEmail(), 5, 150);
    addValidate(Validator.size.getName(), recover.getNif(), 9, 9);
    return generateError();
  }
